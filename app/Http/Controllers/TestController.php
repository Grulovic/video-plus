                ...
                foreach($file_list as $folder) {
                    ...
                    foreach($sub_file_list as $key=>$file_path) {
                        $file_exists = FtpGovFile::
                            where('folder',preg_replace('/[^ \w]+/', '',$folder))
                            ->where('file_path',preg_replace('/[^ \w]+/', '',$file_path))
                            ->first();
//                        echo $folder.') '.$file_path;
                        if(!$file_exists){
                            $new_file = new FtpGovFile();
                            $new_file->folder = preg_replace('/[^ \w]+/', '', $folder);
                            $new_file->file_path = preg_replace('/[^ \w]+/', '', $file_path);
                            $new_file->save();
