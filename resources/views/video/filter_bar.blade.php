 ...
 <form class="form-inline row m-0 p-0" action="/videos" method="GET">
  ...
  <div class="col-lg-3 text-left">
          <div class="input btn-group mb-2">
            ...
            @endphp
            <a href="{{ route('videos.index')}}?{{isset($parameters) ? $parameters : "" }}" class="btn btn-secondary {{ strtok($_SERVER["REQUEST_URI"], '?') == parse_url(route('videos.index'), PHP_URL_PATH) ? "active" : "" }} "><i class="fas fa-th"></i></a>
            <a href="{{ route('videos.list')}}?{{isset($parameters) ? $parameters : "" }}" class="btn btn-secondary {{ strtok($_SERVER["REQUEST_URI"], '?') == parse_url(route('videos.list'), PHP_URL_PATH) ? "active" : "" }}"><i class="fas fa-bars"></i></a>
          @if( auth()->user()->role == "admin")
            <a href="{{ route('videos.create') }}" class="btn btn-success"><i class="fas fa-photo-video"></i> Upload video</a>
          @endif
          </div>
  </div>
  <div class="col-lg-3">
...
<div class="col-lg-6 p-0">
  <div class="row m-0 p-0">
    ...
    <div class="col-5 col-lg-4 pl-0 pr-4 text-right"><div class="btn-group mb-2 ">
      ...
      <a href="/videos" class="btn btn-danger"><i class="fas fa-times"></i></a>
    </div></div>
