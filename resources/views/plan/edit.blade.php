...
<form class="row" action="{{ route('plans.update', $plan->id) }}" method="POST" name="update_plan">
  ...
  <div class="form-group col-lg-4">
    ...
    <div class="form-group col-lg-4">
        <strong>Date</strong>
        <input type="datetime-local" name="date" class="form-control" placeholder="Enter date" value="{{ date_format(date_create($plan->date),'Y-m-d')."T".date_format(date_create($plan->date),'h:m:s') }}">
        <span class="text-danger">{{ $errors->first('location') }}</span>
    </div>
