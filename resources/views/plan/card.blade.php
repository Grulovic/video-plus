          ...
          <div class="btn-group ">
              ...
              @if (!Auth::guest())
                  ...
                  <a href="{{ route('plans.favorite',$plan->id)  }}" type="button" id="plan-favorite-btn-{{$plan->id}}"
                     class="plan-favorite-btn  btn btn-sm  {{ $plan->inFavorite() ? "btn-warning" : "btn-info" }}  pl-4 pr-4" style="border-radius: 0 0.25rem 0.25rem 0;"  data-toggle="tooltip" data-placement="top" title="Get Email Notifications"
                          data-planid="{{$plan->id}}">
